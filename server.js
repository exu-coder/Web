...
async function getDb() {
  ...
  if (!mongoPromise) {
    mongoClient = new MongoClient(MONGODB_URI, { serverSelectionTimeoutMS: 10000, maxPoolSize: 10 });
...
async function applicationsCollection() { return (await getDb()).collection('applications'); }
...
function safePath(base,requestPath){
  ...
  const root=path.resolve(base), target=path.resolve(root,'.'+decoded);
  return target===root||target.startsWith(root+path.sep)?target:null;
}

function sendFile(req,res,file){
  fs.stat(file,(err,stat)=>{
    if(err||!stat.isFile()){res.writeHead(err?.code==='ENOENT'?404:500,{'Content-Type':'text/plain; charset=utf-8'});return res.end(err?.code==='ENOENT'?'Not found':'Server error');}
...
async function initMongo(){
  ...
  // Migrate older records that used createdAt/updatedAt and did not have the public id field.
  ...
  await collection.updateMany(
    { id: { $exists: false } },
    [{ $set: { id: { $concat: ['FITC-', { $toUpper: { $substrBytes: [{ $toString: '$_id' }, 0, 8] } }] } } }]
  );
  await collection.updateMany(
    { status: { $exists: false } },
    { $set: { status: 'new' } }
...
const server=http.createServer(async(req,res)=>{
  ...
  if(normalized==='/health'){
    try{await getDb();return sendJson(res,200,{ok:true,database:'mongodb',database_name:MONGODB_DATABASE,user:'/',admin:'/farabi@/'});}
    catch(error){console.error('MongoDB health check failed:',error.message);return sendJson(res,503,{ok:false,database:'mongodb',error:'Database unavailable'});}
  }

  if(normalized==='/api/applications' && req.method==='GET'){
    try{
      const collection=await applicationsCollection();
      const rows=await collection.find({}).sort({created_at:-1,createdAt:-1}).toArray();
      return sendJson(res,200,rows);
    }catch(error){console.error('MongoDB read failed:',error);return sendJson(res,503,{error:'Database unavailable'});}
  }
  ...
  if(normalized==='/api/applications' && req.method==='POST'){
    try{
      ...
      const app={
        ...
        full_name,phone,email:b.email?String(b.email).trim():null,course,
        message:b.message?String(b.message).trim():null,status:'new',created_at:new Date().toISOString()
      };
      await (await applicationsCollection()).insertOne(app);
      return sendJson(res,201,app);
    }catch(error){console.error('MongoDB insert failed:',error);return sendJson(res,503,{error:'Database unavailable'});}
  }

  if(normalized.startsWith('/api/applications/') && req.method==='DELETE'){
    try{
      const id=normalized.slice('/api/applications/'.length), collection=await applicationsCollection();
      const result=await collection.deleteOne({$or:[{id},{phone:id},{_id:(()=>{try{return require('mongodb').ObjectId.isValid(id)?new (require('mongodb').ObjectId)(id):null}catch{return null}})()}]});
      if(!result.deletedCount)return sendJson(res,404,{error:'Application not found'});
      return sendJson(res,200,{ok:true});
    }catch(error){console.error('MongoDB delete failed:',error);return sendJson(res,503,{error:'Database unavailable'});}
  }

  if(normalized==='/farabi@' || normalized==='/farabi@/')return sendFile(req,res,ADMIN_INDEX);
  if(normalized.startsWith('/farabi@/')){
    const relative=normalized.slice('/farabi@'.length)||'/', file=safePath(path.join(PUBLIC_DIR,'admin'),relative);
...
(async()=>{try{await initMongo();server.listen(PORT,'0.0.0.0',()=>console.log(`Farabi server on ${PORT} | MongoDB: ${MONGODB_DATABASE}`));}catch(error){console.error('Startup failed:',error.message);process.exit(1);}})();